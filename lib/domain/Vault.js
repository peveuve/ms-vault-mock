...
class Vault extends EventEmitter {
  ...
  getKeys (quantity = 25, startIndex = 0) {
    const keysFound = this.keys.slice(startIndex, startIndex + quantity)
    const clonedKeys = cloneDeep(keysFound)
    if (startIndex + quantity < this.keys.length) {
      clonedKeys.nextIndex = startIndex + quantity
    }
    return clonedKeys
  }
}
